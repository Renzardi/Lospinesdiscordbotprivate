...
module.exports = async (client, message) => {
  try {
     ...
     if (cmd.length === 0){
      if(matchedPrefix.includes(client.user.id))
        return message.channel.send(new Discord.MessageEmbed()
          ...
          .setTitle(`Hola? Gracias por tagearme!`)
          .setDescription(`Para ver todos los comandos usa: \`${prefix}help\``)
        );
      return;
      }
        ...
        if (timestamps.has(message.author.id)) { //if the user is on cooldown
          ...
          if (now < expirationTime) { //if he is still on cooldonw
            ...
            return message.channel.send(new Discord.MessageEmbed()
              ...
              .setFooter(ee.footertext,ee.footericon)
              .setTitle(`❌ Porfavor espera ${timeLeft.toFixed(1)} estos segundos para volver a usar este \`${command.name}\` comando.`)
      ...
      try{
        ...
        if(command.memberpermissions && !message.member.hasPermission(command.memberpermissions, { checkAdmin: command.adminPermOverride, checkOwner: command.adminPermOverride })) {
          return message.channel.send(new Discord.MessageEmbed()
            .setColor(ee.wrongcolor)
            .setFooter(ee.footertext, ee.footericon)
            .setTitle("❌ Error | No tienes permitido usar este comando!")
            .setDescription(`Necesitas permisos: \`${command.memberpermissions.join("`, ``")}\``)
          ).then(msg=>msg.delete({timeout: 5000}).catch(e=>console.log("Mensaje Inborrable".gray)));
        }
        ...
        if(!message.guild.me.hasPermission(required_perms)){
          ...
          return message.channel.send(new Discord.MessageEmbed()
            .setColor(ee.wrongcolor)
            .setFooter(ee.footertext, ee.footericon)
            .setTitle("❌ Error | No tengo permisos!")
            .setDescription("Porfavor necesito el rango de `ADMINISTRATOR`")
          )
        }
        ...
        return message.channel.send(new Discord.MessageEmbed()
          .setColor(ee.wrongcolor)
          .setFooter(ee.footertext, ee.footericon)
          .setTitle("❌ Ocurrio un error al usar este: `" + command.name + "` comando")
          .setDescription(`\`\`\`${e.message}\`\`\``)
        ).then(msg=>msg.delete({timeout: 5000}).catch(e=>console.log("Inborrable".gray)));
      }
    }
    ...
    return message.channel.send(new Discord.MessageEmbed()
      .setColor(ee.wrongcolor)
      .setFooter(ee.footertext, ee.footericon)
      .setTitle(`❌ Comando no encontrado, Usa: **\`${prefix}help\`**`)
      .setDescription(`Para obtener informacion especifica, usa \`${prefix}help [COMANDO]\``)
    ).then(msg=>msg.delete({timeout: 5000}).catch(e=>console.log("Inborrable".gray)));
  }catch (e){
    return message.channel.send(
    new MessageEmbed()
    .setColor("RED")
    .setTitle(`❌ ERROR | Algo fuera de lo normal...`)
