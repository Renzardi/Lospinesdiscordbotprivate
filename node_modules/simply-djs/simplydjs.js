...
module.exports = {
    ...
    tictactoe: async function (message, options = []) {
        ...
        if (opponent.id === message.member.id) return message.channel.send("Este juego es solo para 2 personas!");


        if (!opponent) return message.channel.send("Menciona a un usuario para jugar al TATETI!")
        ...
        if (options.credit === false) {
            foot = options.embedFoot || 'Asegura tu victoria! ;)'
        } else {
            foot = 'LosPibes | 2.0'
        }
        ...
        const xoemb = new Discord.MessageEmbed()
            .setTitle('TATETI')
            .setDescription(`**Como juego ?**\n*Espera tu turno.. Cuando es tu turno, Clickea el boton en el espacio que este libre!.*`)
            .setColor(options.embedColor || 0x075FFF)
            .setFooter(foot)
            .setTimestamp()
        ...
        let msg = await message.channel.send(`Esperando al jugador... | <@!${Args.userid}>, Tu forma: ${o_emoji}`)
        tictactoe(msg)
        async function tictactoe(m) {
            ...
            if (won["<:O_:863314110560993340>"] != false) return m.edit(`<@!${Args.userid}> (${o_emoji}) Gano.. ¡Buen Juego!.`)
            ...
            if (won["<:X_:863314044781723668>"] != false) return m.edit(`<@!${Args.userid}> (${x_emoji}) Gano.. ¡Buen Juego!.`)
            ...
            m.edit(`Esperando al jugador... | <@!${Args.userid}> | Tu Forma: ${Args.user == 0 ? `${o_emoji}` : `${x_emoji}`}`, buttons)
            ...
            collector.on('end', collected => {
                if (collected.size == 0) m.edit(`<@!${Args.userid}> El juego finalizo por que el rival no presento acciones en (30s)`)
            });
        }
    },
    ...
    calculator: async function (message, options = []) {
        ...
        if (options.credit === false) {
            foot = message.guild.name, message.guild.iconURL()
        } else {
            foot = 'LosPibes | 2.0'
        }
        ...
        client.on("clickButton", async b => {
            ...
            if (b.message.id == m.id && b.clicker.user.id == message.author.id) {
                if (b.id == "back_button_embed") {
                    ...
                    b.message.delete()
                    b.reply.send('Message Delated').then((m) => {
                        setTimeout(() => {
                            m.delete()
                        }, 5000)
                    })
...
let membero = button.guild.members.cache.get(button.clicker.user.id)

button.channel.updateOverwrite(membero, { VIEW_MESSAGES: true, SEND_MESSAGES: false });
...
button.channel.updateOverwrite(membero, { VIEW_MESSAGES: true, SEND_MESSAGES: true });
    ...
    message.channel.send({ embed: embedo, component: row1 }).then((m) => {
        const filter = (button) => button.clicker.user.id === message.author.id
        const collect = m.createButtonCollector(filter, { max: 1, time: 15000 });

        collect.on("collect", async b => {

            if(b.id === 'send-sug'){
                b.reply.send('Ok Suggested.', true);
                b.message.delete();
            ...
            ch.send(emb, { component: row })
            } else if(b.id === 'nope-sug') {
                b.reply.defer();
                b.message.edit('Ok i didnt send the suggestion', { components: [] })
            }
        ...
        collect.on("end", async b => {
            if (b.size == 0){
            m.delete()
            m.channel.send('Timeout.. So I didnt send the suggestion.')
            }
        })
   ...
   suggestBtn: async function (button, users, options = []) {
    ...
    rps: async function(message, options = []){
        ...
        let decline = new disbut.MessageButton()
        .setStyle('red')
        .setID('decline')
        .setLabel('Decline')
        ...
        collector.on('collect', (button) => {
            ...
            .setStyle(options.rockColor || 'grey')
            .setID('rock')
            .setLabel('Rock')
            .setEmoji('🪨')

            let paper = new disbut.MessageButton()
            .setStyle(options.paperColor || 'grey')
            .setID('paper')
            .setLabel('Paper')
            .setEmoji('📄')

            let scissors = new disbut.MessageButton()
            .setStyle(options.scissorsColor || 'grey')
            .setID('scissors')
            .setLabel('Scissors')
            .setEmoji('✂️')
    ...
    .setTitle('Challenge Not Accepted')
                ...
                .setFooter(foot)
                .setDescription(`The user (${opponent.user}) declined your request..`)
            ...
            collect.on('end', c => {
                ...
                if(mem == 'rock' && auth == 'scissors'){
                    ...
                    let embed = new Discord.MessageEmbed()
                    .setTitle('Draw!')
                    .setColor(options.drawEmbedColor || 0x075FFF)
